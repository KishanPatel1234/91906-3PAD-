...
class MainApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title('IKRAAMS BENCH SIMULATOR'efe) # Window title.
    ...
    def open_start_window(self): # Selection of weight window.
        ...
        select_game_button = tk.Button(start_window, text='SELECT', bg="#E85D04", fg="#FFFFFF", font=("Arial", 10), command=lambda: self.open_game_window(current_value.get()))
        select_game_button.place(x=100, y=240)
    ...
    def open_game_window(self, weight):
        ...
        main_canvas = tk.Canvas(game_window, width=400, height=300)
        main_canvas.place(y=0, x=0)

        question_label = tk.Label(game_window, bg='#333333', fg='#FFFFFF', font=("Arial", 18))
        question_label.place(y=20)

        feedback_label = tk.Label(game_window, bg='#333333', fg='#FFFFFF', font=("Arial", 14))

        frame2 = tk.PhotoImage(file="bench.gif", format="gif -index 2")
        main_canvas.create_image(0, 0, anchor="nw", image=frame2)
        
        #correct_image = tk.PhotoImage(file="images/up.png")

        #incorrect_image = tk.PhotoImage(file="images/down.png")
        #correct_image=r"C:\Users\20178\OneDrive - Lynfield College\13PAD\AS91906 and AS91907\3PAD Assessment\up.png"
        #correct=PhotoImage(file=correct_image)
        #up=tk.Label(game_window,image=correct)
        #up.place(x=0,y=0)

        #correct_image=Image.open(os.getcwd()+r"\up.png")
        #correct_image=Image                     
